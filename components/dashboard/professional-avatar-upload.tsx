...
import { updateProfessionalAvatar } from "@/app/dashboard/professionals/[id]/actions"
...
export function ProfessionalAvatarUpload({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/upload/professional-avatar", {
        method: "POST",
        body: formData,
      })
      if (!res.ok) throw new Error()
      const { url } = await res.json()
      await updateProfessionalAvatar(professionalId, url)
      setPreview(url)
      router.refresh()
    } catch {
